...
- 部署 `UUPSDemoLogicV1`
- 用 `ERC1967Proxy` 包一层
- 在代理构造时完成 `initialize(owner)`，代币余额和业务数值从默认值 `0` 开始
- 通过实现合约暴露的 `upgradeToAndCall` 完成升级
- 代理 owner 可以通过 `mint(target, amount)` 向目标地址铸造代币
...
4. 通过代理调用 `setValue`、`add`、`mint` 等函数：
   - 代理只做转发
   - 业务逻辑在实现合约里执行
   - 状态仍然落在代理上
   - `mint(target, amount)` 只能由代理 owner 调用，`amount` 使用 18 位精度的最小单位
...
7. V2 通过 `initializeToken` 这个 `reinitializer(2)` 补充 ERC20 元数据；升级脚本会在切换 implementation 的同一笔交易中执行它。
